...
int main(int argc, char * argv[]) {
	...
	while ((opt = getopt(argc, argv, "d:")) != -1) {
		...
		case 'd':
			...
			break;
		default: /* '?' */
			fprintf(stderr, "Usage: %s [-d debug-level] name\n", argv[0]);
			return -1;
		}
	}
	...
	while (fseek(f, 2*pc*sizeof(int), SEEK_SET) == 0 && fread(cmd, sizeof(int), 2, f) == 2) {
		...
		case c_loadc:
			...
			break;
		case c_inc:
			pop(&ops, &op_re), push(&ops, op_re + cmd[1]);
			break;
		case c_dec:
			pop(&ops, &op_re), push(&ops, op_re - cmd[1]);
			break;
		case c_op_add:
			pop(&ops, &op_re), pop(&ops, &op_li), push(&ops, op_li + op_re);
			break;
		case c_op_sub:
			pop(&ops, &op_re), pop(&ops, &op_li), push(&ops, op_li - op_re);
			break;
		...
		case c_drop:
			...
			break;
		default:
			printf("Error 0 pc=%d %4d %4d", pc, cmd[0], cmd[1]), pc=-10;
		}
		debug();
		pc++;
	}
